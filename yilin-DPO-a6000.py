...
# image_folder="/data/yilin/MM-RLHF-Dataset/"
...
def test_mm_dpo():
    ...
    # lrs = [5e-6, 5e-7, 1e-7]
    processes = []
    master_ports=[60031,60011,60012]
    ...
    # for time in range(times):
            ...
            master_port = master_ports[i]
            i += 1
            run_name=f"no-cn-mcq-lr-{lr}-acc_batch-{effective_batch}-beta-{beta}-use_anchor-{use_anchor}-{time}"
            pretrained=f"/home/yilin/yilin-DPO/output/{base_model}/{run_name}"

            cmd = [
                # f"""python -m debugpy --connect 5679 $(which deepspeed) --include=localhost:0 --master_port {master_port} train_rdpo.py \
                f"""deepspeed --include=localhost:4,5 --master_port {master_port} train_rdpo.py \
                --model_name_or_path {model_name} \
                --data_path {data_path} \
                --deepspeed "./deepspeed/zero2.json" \
                --per_device_train_batch_size 1 \
...
def test_mdpo_dpo():
    ...
    processes = []
    num_train_epochs = 2
    master_ports=[60032,60011,60012]
    for time, lr, use_anchor in itertools.product(range(times), lrs, [False]):
    # for time in range(times):
            if i >= len(master_ports):
                raise ValueError(f"i={i} 超出 master_ports 长度 {len(master_ports)}")
            master_port = master_ports[i]
            i += 1
            run_name=f"mdpo-silkie-lr-{lr}-acc_batch-{effective_batch}-beta-{beta}-use_anchor-{use_anchor}-epoch-{num_train_epochs}-{time}"
            pretrained=f"/home/yilin/yilin-DPO/output/{base_model}/{run_name}"
...
def test_re_align_masked_dpo():
    i = 0
    use_anchor=False
    data_path="./preference_data/masked_pooler.json"
    image_folder="/data/yilin/train2014/"
    times = 10
    effective_batch = 12
    # lrs = [1e-4, 5e-5]
    ...
    processes = []
    num_train_epochs = 1
    master_ports=[60052,60011,60012]
    for time, lr, use_anchor in itertools.product(range(5, times), lrs, [False, True]):
    # for time in range(times):
            if i >= len(master_ports):
                raise ValueError(f"i={i} 超出 master_ports 长度 {len(master_ports)}")
            master_port = master_ports[i]
            i += 1
            run_name=f"masked-lr-{lr}-acc_batch-{effective_batch}-beta-{beta}-use_anchor-{use_anchor}-{time}"
            pretrained=f"/home/yilin/yilin-DPO/output/{base_model}/{run_name}"
